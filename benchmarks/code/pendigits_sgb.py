...
def experiment_sgb():
    ...
    stream = NumpyStream(X_r, y_r, dataset_name="pendigits")
    ...
    for i in range(7400):
        ...
        if i > 0 and (i + 1) % 100 == 0:
            ...
            p_t = 0.0
            test_stream = NumpyStream(X_test, y_test, dataset_name="pendigits")
            start_time = time.perf_counter()
...
# Prepare pendigits DNA data
dataset = openml.datasets.get_dataset(1019)
...
if args.all or args.sgb:
    ...
    for i in range(1):
        ...
        write_result(DIRECTORY + "results/sgb/pendigits_acc", sgb_acc_l)
        write_result(DIRECTORY + "results/sgb/pendigits_train_t", sgb_train_t_l)
        write_result(DIRECTORY + "results/sgb/pendigits_test_t", sgb_test_t_l)
        write_result(DIRECTORY + "results/sgb/pendigits_v_m", sgb_v_m_l, True)
        # write_result(DIRECTORY + "results/sgb/pendigits_n_node", sgb_n_node_l)
        # write_result(DIRECTORY + "results/sgb/pendigits_size", sgb_size_l, True)
